...
try {
  if ($Mode -ne 'production') {
    ...
    if ($Mode -in @('phase2a','bcd')) {
      Copy-Item -LiteralPath (Join-Path $PSScriptRoot "sync-$Mode-device/NativeRefresh.ets") -Destination $nativeRefresh
    }
  }
  ...
  if ($Mode -in @('phase2a','bcd') -and (Test-Path -LiteralPath $nativeRefresh)) { Remove-Item -LiteralPath $nativeRefresh }
}
exit $buildExit
