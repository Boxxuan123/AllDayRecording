...
> 文档状态：`current`
> 当前结论：实现和本地验证完成，等待用户 Phone/WATCH 5 快速回归后提交。
...
- 提交前只暂存 Phase 5 源码、质量脚本和文档；签名材料、HAP、测试报告、设备备份和本机配置不得进入 Git。

## 5. 用户回归清单

由于本阶段对正常路径只增加异常边界并保持原调用顺序，建议做一轮短回归后提交：

1. WATCH 5 开始录音、停止封口并本地试听。
2. Phone 执行重连、同步一个缺失文件并试听，确认 Watch 源文件仍保留。
3. 从 Phone 发起一次手表录音控制；若进入通知降级，确认点击通知仍能回到手表前台继续操作。

上述回归需要安装时，仍必须先执行总计划第 4 节的双份只读导出与 SHA-256 清单比较。
